...
int main(int argc, char **argv)
    ...
    BIO       *certs = BIO_new_file("certs\\Lexne.crt", "w");
    BIO       *lkeys = BIO_new_file("certs\\Lexne.key", "w");
    BIO       *keys  = BIO_new_file("certs\\lexneSign.key", "r");
    
    BIO       *imps  = BIO_new_file("certs\\LexneSign.crt", "r");
    BIO       *reqs  = BIO_new_file("certs\\lexne.txt", "w");
	...
	mkcert(&px5s, &x509,&pkey, &nkey,2048,0x66224184,365*6);
...
int mkcert(X509 **x509x,X509 **x509p, EVP_PKEY **pkeyp, 
	...
	*nkeyp=pk;
	...
	X509_NAME_add_entry_by_txt(name,"OU", 
	  MBSTRING_ASC, (unsigned char *)"(C) 2013 LocalSign - SSL CA", -1, -1, 0);
	X509_NAME_add_entry_by_txt(name,"CN",
				MBSTRING_ASC, 
		(unsigned char *)"www.lexne.com", -1, -1, 0);
    #else
    ...
    add_ext(x, NID_subject_alt_name, "DNS:www.lexne.com,DNS:lexne.com,DNS:*.lexne.com");
    add_ext(x, NID_basic_constraints, "CA:FALSE");
	add_ext(x, NID_crl_distribution_points,"URI:http://www.lexne.com/EVS-2048.crl");
    ...
    add_ext(x, NID_info_access , "caIssuers;URI:http://www.lexne.com/CA1-2012.crt,"
                                 "OCSP;URI:http://www.lexne.com:8080/");
    #ifdef __CCS_                             
    ...
    */
    ...
    #ifndef CUSTOM_EXT
	{
		...
		printf("OBJ: %d \n", nid);
		add_ext(x, nid, "https://www.lexne.com/images/localsign.gif");
	}
    #endif
    
    
    #ifdef __STD__
    X509_alias_set1(x, (unsigned char *)"LexneSign, Inc.", 15); 
...
do{  
    ...
    pk = imp->cert_info->key->public_key;
    if (!pk) {
...
X509_EXTENSION *new_cert_policies(char *stringc)
{
    ...
    lstrcpy(url, "https://www.lexne.com/cps");
    ...
    qual->d.usernotice = note;
		...
		lstrcpy(buffer, " LocalSign Trusted Network .\r\n");
		lstrcat(buffer, "\r\n");      
        lstrcat(buffer," Copyright (c) 2012 LocalSign - Lexne, Inc.");
        
    note->exptext = M_ASN1_VISIBLESTRING_new();
...
X509_EXTENSION *new_usage_period(void)
{
	...
	if(usage->notAfter == NULL || !usage->notBefore){
		usage->notAfter = M_ASN1_GENERALIZEDTIME_new();
		usage->notBefore = M_ASN1_GENERALIZEDTIME_new();
	}
